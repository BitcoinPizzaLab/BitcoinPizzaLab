        ...
        {pizzaListFilter.length === 1 && pizzaListFilter.map((item) => {
          return (
            <Flex borderRadius="5px" overflow="hidden" key={item.id} bg='#F2921D' w={['80%', '30%']} m="auto" mt="50px">
              <NftItem hashes={item.hashes} showId={showId} id={item.id} src={`https://bitcoinpizzadatasync.bitcoinnftpizzalab.workers.dev/output/cryptopizza_${item.id}.png`}></NftItem>
            </Flex>
          )
        })}
